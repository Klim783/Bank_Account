...
#         if not v:
#             raise ValueError('Waller name cannot be empty')
#         return v
#
...
class CreateWalletRequest(BaseModel):
    ...
    currency: CurrencyEnum = CurrencyEnum.RUB  # Указано конкретное значение Enum
...
class TransferCreateSchema(BaseModel):
    ...
    @field_validator('to_wallet_id')
    @classmethod
    def wallets_must_differ(cls, v: int, info: ValidationInfo) -> int:
        if 'from_wallet_id' in info.data and v == info.data['from_wallet_id']:
            raise ValueError('Same wallets ids!')
        return v
